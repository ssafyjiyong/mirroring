...
def grab_cut(resize_img, resized):
    ...
    global new_contour
    new_contour = contoursGrab(resize_img, new_edged)
...
def contours(edge, resize_img, src, ckpnt, vertex):
    # global checkpnt
    # global vertex
    ...
    for i in cnts:
        ...
        if len(approx) != 4 and checkpnt == 0:  # 사각형이 그려지지 않는다면 grab_cut 실행
            size = 0
            checkpnt += 1
            grab_cut(resize_img, resize_img)
...
def contoursGrab(resize_img, edged):
...
def transformation(resize_img, src):
    #print(src)
